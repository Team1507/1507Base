...
| **2026 port stress test** | The 2026 robot (6 mechanisms, swerve paste, 7 competition autos) ported onto the base in a throwaway working tree and run in sim on Blue, Red and Seed-Left. All 7 routines ran to completion, aimed within 1–2°, and flipped/mirrored exactly; our code 0.8 ms/loop. `SwerveConfigTest` caught the real 2026 kV = 2.75 bug. Base fixes from it: `.holdUntil()` (hold at a checkpoint until an action finishes, like 2026's `parallel(moveThrough, deploy)`), every CANivore in use is logged (not just the default bus), and the gear-ratio-changes-gain-units warning in `MotorConfig` and the wiki. The port code was not committed |
