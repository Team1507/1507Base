...
> **Status (Swerve split):** the paste zone, motor configs and build checks moved from `Swerve.java` to `SwerveConfig.java`. The line links in the findings below point at `Swerve.java` as it was when audited.
